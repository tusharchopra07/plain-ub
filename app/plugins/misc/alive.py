...
async def alive(bot: BOT, message: Message):
    ...
    if bot.is_user and hasattr(bot, "bot") and bot.bot:
        ...
        await bot.send_inline_bot_result(
            chat_id=message.chat.id, result_id=inline_result.results[0].id, query_id=inline_result.query_id
        )
        return
...
if _bot.is_bot:
    ...
    async def return_inline_alive_results(client: BOT, inline_query: InlineQuery):
        kwargs = dict(title="Send Alive Media.", caption=await get_alive_text(), reply_markup=get_alive_buttons(client))
...
async def get_alive_text() -> str:
    ...
    return (
        ...
        f"\n › Python        :   <code>v{python_version()}</code>"
